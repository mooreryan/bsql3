...
pub fn syntax_error_test() {
  let assert Ok(db) = bsql3.new_database(":memory:")

  let assert Error(bsql3.SqliteError(code: result_code.Error, message: _)) =
    bsql3.exec(db, "this won't work!")

  let assert Ok(Nil) = bsql3.close(db)
}
...
pub fn basic_usage_test() {
  ...
  let assert Ok(insert_statement) =
    bsql3.prepare(db, "INSERT INTO users (name, age) VALUES (?, ?)")

  let assert Ok(bsql3.RunInfo(changes: 1, last_insert_row_id: 1)) =
    bsql3.run(insert_statement, [bsql3.text("Alice"), bsql3.int(30)])

  let assert Ok(all_users_statement) = bsql3.prepare(db, "SELECT * FROM users")

  let assert Ok([user]) = bsql3.all(all_users_statement, [], user_decoder())
...
pub fn bad_decoder_test() {
  ...
  let assert Error(bsql3.DecodeError(_)) =
    bsql3.all(all_users_statement, [], bad_user_decoder())
...
pub fn db_close_test() {
  let assert Ok(db) = bsql3.new_database(":memory:")
  let assert Ok(Nil) = bsql3.close(db)
  let assert Error(bsql3.JsError(_, _)) = bsql3.exec(db, "SELECT 1")
}
...
pub fn db_close_prepared_statement_test() {
  let assert Ok(db) = bsql3.new_database(":memory:")
  let assert Ok(stmt) = bsql3.prepare(db, "select 1")
  let assert Ok(Nil) = bsql3.close(db)
  let assert Error(bsql3.JsError(
    name: "TypeError",
    message: "The database connection is not open",
  )) = bsql3.all(stmt, [], decode.field("1", decode.int, decode.success))
}
...
pub fn pragma_foreign_key_test() {
  let assert Ok(db) = bsql3.new_database(":memory:")
  let assert Ok(Nil) = bsql3.pragma(db, "foreign_keys = ON")
  let assert Ok(Nil) =
    bsql3.exec(
      db,
  ...
  let assert Error(bsql3.SqliteError(
    code: result_code.ConstraintForeignkey,
    message: "FOREIGN KEY constraint failed",
  )) = bsql3.exec(db, "INSERT INTO posts (user_id) VALUES (1234)")

  // Turn the foreign keys off and we can insert
  let assert Ok(Nil) = bsql3.pragma(db, "foreign_keys = OFF")
...
pub fn kitchen_sink_test() {
  ...
  let assert Ok(_) = bsql3.run(create_users_table, [])

  // You can use db exec directly to insert users, but you probably shouldn't
  // since you can't get the rowid back....
  let assert Ok(Nil) =
    bsql3.exec(db, "insert into users (name, age) values ('Ash', 29)")

  // Inserting users with statement.run
  let assert Ok(insert_user) =
    bsql3.prepare(db, "insert into users (name, age) values (?, ?)")
  let assert Ok(bsql3.RunInfo(changes: 1, last_insert_row_id: 2)) =
    bsql3.run(insert_user, [bsql3.text("Misty"), bsql3.int(31)])

  // Inserting users while returning values
  let assert Ok(insert_user_returning) =
    bsql3.prepare(db, "insert into users (name, age) values (?, ?) returning *")
  let assert Ok([#(3, "Brock", 35)]) =
    bsql3.all(
      insert_user_returning,
      [bsql3.text("Brock"), bsql3.int(35)],
      user_decoder,
    )

  let assert Ok(select) = bsql3.prepare(db, "select * from users where age > ?")

  // Raw returns JS arrays rather than objects, but you must explicitly opt into
  // it.
  let assert Ok(select) = bsql3.raw(select, True)
  let assert Ok(users) = bsql3.all(select, [bsql3.int(30)], raw_user_decoder)
  assert users == [#(2, "Misty", 31), #(3, "Brock", 35)]

  // Switching back to non-raw causes the statement to return JS objects
  // instead.
  let assert Ok(select) = bsql3.raw(select, False)
  let assert Ok(users) = bsql3.all(select, [bsql3.int(30)], user_decoder)
  assert users == [#(2, "Misty", 31), #(3, "Brock", 35)]

  let assert Ok(Some(#(2, "Misty", 31))) =
    bsql3.get(select, [bsql3.int(30)], user_decoder)
  let assert Ok(None) = bsql3.get(select, [bsql3.int(50)], user_decoder)

  // Counting rows
  let assert Ok(count_users) = bsql3.prepare(db, "select count(*) from users")
  let assert Ok([3]) =
    bsql3.all(
      count_users,
      [],
...
pub fn raw_cannot_be_called_on_a_statement_that_doesnt_return_data_test() {
  let assert Ok(db) = bsql3.new_database(":memory:")
  let assert Ok(create_users_table) =
    bsql3.prepare(db, "create table users (name text, age int)")
  let assert Ok(_) = bsql3.run(create_users_table, [])
  let assert Ok(insert_user) =
    bsql3.prepare(db, "insert into users (name, age) values (?, ?)")
...
pub fn coerce_roundtrip_test() {
  ...
  let decoder = {
    ...
    use a_bool <- decode.field(2, decode.optional(bsql3.decode_bool()))
  ...
  let assert Ok(db) = bsql3.new_database(":memory:")
  let assert Ok(stmt) = bsql3.prepare(db, "select ?, ?, ?, ?, ?")
  let assert Ok(stmt) = bsql3.raw(stmt, True)
  ...
  let Nil = {
    ...
    let assert Ok([row]) =
      bsql3.all(
        stmt,
        [
          bsql3.nullable(bsql3.int, an_int),
          bsql3.nullable(bsql3.real, a_float),
          bsql3.nullable(bsql3.bool, a_bool),
          bsql3.nullable(bsql3.text, some_text),
          bsql3.nullable(bsql3.blob, a_blob),
        ],
        decoder,
      )
...
pub fn database_properties_test() {
  ...
  assert bsql3.database_open(db) == True
  assert bsql3.database_in_transaction(db) == False
  assert bsql3.database_name(db) == ":memory:"
  assert bsql3.database_memory(db) == True
  assert bsql3.database_readonly(db) == False
...
pub fn readonly_db_test() {
  ...
  let assert Ok(db) = bsql3.new_database(db_name)
  let assert Ok(Nil) = bsql3.close(db)

  let assert Ok(db) =
    bsql3.configure(db_name)
    |> bsql3.readonly(True)
    |> bsql3.build()
  ...
  )) = bsql3.exec(db, "create table users (name text)")
...
pub fn statement_properties_test() {
  ...
  assert bsql3.statement_database(stmt) == db
  assert bsql3.statement_source(stmt) == "select 1"
  assert bsql3.statement_reader(stmt) == True
  assert bsql3.statement_readonly(stmt) == True
...
pub fn readme_example_test() {
  ...
  let assert Ok(insert_user) =
    bsql3.prepare(db, "insert into users (name, age) values (?, ?)")
  let assert Ok(bsql3.RunInfo(changes: 1, last_insert_row_id: 1)) =
    bsql3.run(insert_user, [bsql3.text("Ash"), bsql3.int(29)])
  ...
  //
  let assert Ok(users) = bsql3.all(select, [bsql3.int(50)], user_decoder())
  assert users == []
